  ...
  return <div className="space-y-6 animate-fade">
    ...
    <div className="grid lg:grid-cols-[280px_1fr] gap-4">
      ...
      <section className="card p-5 sm:p-6">
        {!selected ? <div className="max-w-xl space-y-5"><div><div className="flex items-center gap-2 text-brass"><Sparkles size={16}/><span className="readout">NEW STORY</span></div><h2 className="font-display lowercase text-2xl text-ink mt-2">Start an analysis</h2></div><div className="grid sm:grid-cols-[1fr_150px] gap-3"><input className="input-field" placeholder="Movie, series, anime or story title" value={title} onChange={e=>setTitle(e.target.value)} onKeyDown={e=>e.key==='Enter'&&create()}/><select className="input-field" value={kind} onChange={e=>setKind(e.target.value)}>{kinds.map(k=><option key={k}>{k}</option>)}</select></div><div className="grid sm:grid-cols-2 gap-3"><select className="input-field" value={angle} onChange={e=>setAngle(e.target.value)}>{angles.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><input className="input-field" placeholder="Optional question: Why did this happen?" value={question} onChange={e=>setQuestion(e.target.value)}/></div><button className="btn-primary" disabled={!title.trim()||creating} onClick={create}>{creating?'creating…':'create project'}</button></div> : <div className="space-y-6"><div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3"><div><p className="readout">{selected.kind.toUpperCase()} · {selected.status.toUpperCase()}</p><h2 className="font-display lowercase text-2xl text-ink mt-1">{selected.title}</h2></div><button className="btn-primary" disabled={loading} onClick={analyze}>{loading?'analyzing…':'analyze story'}</button></div><div><label className="readout block mb-2">SOURCE TEXT OR TRANSCRIPT</label><textarea className="input-field min-h-48 w-full resize-y" placeholder="Paste plain text or SRT/VTT source text. Timestamped text becomes exact evidence; plain text remains explicitly unlocated." value={transcript} onChange={e=>setTranscript(e.target.value)}/><div className="flex gap-2 mt-2"><button className="btn-ghost" disabled={loading||!transcript.trim()} onClick={addTextSource}><FileText size={14}/> add as source</button><span className="text-xs text-muted self-center">{selected.sources?.length || 0} source(s) attached</span></div><div className="mt-4 rounded-input border border-rule p-3 space-y-3"><div className="flex items-center justify-between"><label className="readout block">UPLOAD SOURCE</label><span className="text-[10px] text-muted text-right max-w-[300px]">Video: MP4, MKV, MOV, AVI, WebM, M4V · Audio: MP3, WAV, M4A, AAC, FLAC, OGG · Documents: PDF, SRT, VTT, TXT, MD, JSON</span></div><input ref={fileInputRef} type="file" accept="video/*,audio/*,.mp4,.mkv,.mov,.avi,.webm,.m4v,.mp3,.wav,.m4a,.aac,.flac,.ogg,.pdf,.srt,.vtt,.txt,.md,.json" onChange={e=>setUpload(e.target.files?.[0] || null)} className="absolute w-px h-px opacity-0 pointer-events-none" tabIndex={-1}/><div role="button" tabIndex={0} onClick={()=>fileInputRef.current?.click()} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();fileInputRef.current?.click();}}} onDragOver={e=>{e.preventDefault();e.stopPropagation();e.currentTarget.classList.add('border-brass')}} onDragLeave={e=>{e.preventDefault();e.stopPropagation();e.currentTarget.classList.remove('border-brass')}} onDrop={e=>{e.preventDefault();e.stopPropagation();e.currentTarget.classList.remove('border-brass');const file=e.dataTransfer.files?.[0];if(file&&!loading)setUpload(file);}} className="rounded-input border border-dashed border-rule p-5 text-center cursor-pointer hover:border-brass transition-colors"><div className="flex justify-center mb-2"><Plus size={18}/></div><p className="text-sm text-ink2">Click to choose a file or drag it here</p><p className="text-[11px] text-muted mt-1">{upload ? upload.name : 'MKV and other supported video/audio files are accepted'}</p></div><div className="flex flex-wrap items-center gap-3"><label className="text-xs text-muted flex items-center gap-2"><input type="checkbox" checked={transcribe} onChange={e=>setTranscribe(e.target.checked)}/> transcribe video/audio with local ASR</label><button className="btn-ghost" disabled={loading||!upload} onClick={uploadSource}><FileText size={14}/> upload source</button>{upload && <button type="button" className="btn-ghost" disabled={loading} onClick={()=>{setUpload(null);if(fileInputRef.current)fileInputRef.current.value='';}}>clear</button>}</div><p className="text-[11px] text-muted">Choose or drag a source first, then click <strong>upload source</strong> to attach it to this project.</p></div></div>{selected.analysis && <div className="grid md:grid-cols-2 gap-4"><div className="rounded-input border border-rule p-4"><div className="flex items-center gap-2 text-brass mb-2"><BookOpen size={15}/><span className="readout">STORY</span></div><p className="text-sm text-ink2 leading-relaxed">{selected.analysis.summary}</p><p className="text-xs text-muted mt-2">Lens: {selected.brief?.angle || 'story'}{selected.brief?.question ? ' · '+selected.brief.question : ''}</p>
