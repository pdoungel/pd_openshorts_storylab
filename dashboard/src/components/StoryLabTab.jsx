...
</div><p className="text-xs text-muted mt-3">{selected.analysis.story?.conflict}</p></div><div className="rounded-input border border-rule p-4"><div className="flex items-center gap-2 text-brass mb-2"><Lightbulb size={15}/><span className="readout">THEORIES</span></div>{(selected.analysis.theories || []).map(t => (<div key={t.id} className="mb-3"><div className="text-sm text-ink">{t.title}</div><p className="text-xs text-muted mt-1">{t.claim}</p></div>))}</div></div><div className="space-y-3">
