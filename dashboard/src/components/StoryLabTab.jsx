...
export default function StoryLabTab() {
  ...
  const searchScenes = async () => {
    if (!selected) return;
    setLoading(true); setError('');
    try { replaceProject(await apiJson('/api/storylab/projects/' + selected.id + '/scenes/search', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({})})); }
  ...
  const extractScenes = async () => {
    if (!selected) return;
    setLoading(true); setError('');
    try { replaceProject(await apiJson('/api/storylab/projects/' + selected.id + '/scenes/extract', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scene_ids:(selected.scenes||[]).filter(s=>s.extraction_status==='candidate').map(s=>s.id)})})); }
...
</div><p className="text-xs text-muted mt-3">{selected.analysis.story?.conflict}</p></div><div className="rounded-input border border-rule p-4"><div className="flex items-center gap-2 text-brass mb-2"><Lightbulb size={15}/><span className="readout">THEORIES</span></div>{selected.analysis.theories?.length ? selected.analysis.theories.map(t=><div key={t.id} className="mb-3"><div className="text-sm text-ink">{t.title}</div><p className="text-xs text-muted mt-1">{t.claim}</p></div>) : <p className="text-xs text-muted">Interpretation stays separate from evidence.</p>}</div></div><div className="space-y-3">
...
</div>}{selected.analysis && <div className="space-y-3">
  <div className="flex items-center justify-between"><span className="readout">SCENE RESEARCH</span><div className="flex gap-2"><button className="btn-ghost" disabled={loading} onClick={searchScenes}>find source scenes</button>{selected.scenes?.some(s=>s.extraction_status==='candidate') && <button className="btn-primary" disabled={loading} onClick={extractScenes}>extract {selected.scenes.filter(s=>s.extraction_status==='candidate').length} scene(s)</button>}</div></div>
  {selected.scenes?.length ? <div className="grid md:grid-cols-2 gap-3">{selected.scenes.map(scene=><div key={scene.id} className="rounded-input border border-rule p-3"><div className="flex justify-between gap-2"><div className="text-sm text-ink">{scene.title}</div><span className="text-[10px] uppercase text-muted">{scene.extraction_status}</span></div><div className="text-xs text-muted mt-1">{scene.start.toFixed(3)}s — {scene.end.toFixed(3)}s · relevance {Math.round(scene.relevance*100)}%</div><p className="text-xs text-ink2 mt-2">{scene.purpose}</p><div className="flex flex-wrap gap-1 mt-2">{scene.evidence_ids?.map(id=><span key={id} className="text-[10px] rounded border border-rule px-1.5 py-0.5 text-muted">{id.slice(-6)}</span>)}</div>{scene.output_file && <p className="text-[10px] text-muted mt-2 break-all">{scene.output_file}</p>}</div>)}</div> : <p className="text-xs text-muted">Analyze the project, then Story Lab can surface every timestamp-backed source moment and extract approved working clips.</p>}
